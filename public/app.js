        ...
        function calcHealthScore() {
            ...
            if (_pctEl) {
                ...
                if (age >= 18 && age <= 80) {
                    ...
                    if (_pctHL) _pctHL.textContent = _t('hs.pct.headline').replace('{pct}', _pg.pct).replace('{bracket}', _pg.label);
                    if (_pctNm) { _pctNm.textContent = hsOrdinal(_pg.pct); _pctNm.style.color = _pc; }
                    if (_pctCx) _pctCx.textContent = _pg.pct >= 80 ? _t('hs.pct.top')
                                                   : _pg.pct >= 60 ? _t('hs.pct.above')
                                                   : _pg.pct >= 40 ? _t('hs.pct.median')
                                                   : _t('hs.pct.below');
                } else {
                    if (_pctHL) _pctHL.textContent = _t('hs.pct.prompt');
                    if (_pctNm) { _pctNm.textContent = '?'; _pctNm.style.color = '#94a3b8'; }
                    if (_pctCx) _pctCx.textContent  = _t('hs.pct.enter.age');
                }
            }
