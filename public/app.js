        ...
        const _mobTabMeta = {
            ...
            returnscalc: { label: 'Returns Calculator', icon: '🔍' },
            ...
            'dashcat-situation': { label: 'For You',        icon: '🎯' },
            userguide:      { label: 'User Guide',     icon: '📖' },
        ...
        function switchMode(mode) {
            ...
            function _doReveal() {
                ...
                if (isDashSituation) { applyLang(); return; }
                ...
                if (isFireAge)       { if (typeof initFireAge === 'function') initFireAge(); applyLang(); return; }
        ...
        function getInvestmentSuggestions(years) {
            if (years < 7) {
                return [
                    { name:_t('inv.s.fd.name'),    icon:'🏦', returns:'6.5 – 8%',       risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.fd.why'),    what:_t('inv.s.fd.what'),    scenarios:_t('inv.s.fd.scenarios'),    example:_t('inv.s.fd.example') },
                    { name:_t('inv.s.rd.name'),    icon:'📅', returns:'6 – 7.5%',        risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.rd.why'),    what:_t('inv.s.rd.what'),    scenarios:_t('inv.s.rd.scenarios'),    example:_t('inv.s.rd.example') },
                    { name:_t('inv.s.lmf.name'),   icon:'💧', returns:'5.5 – 7%',        risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.lmf.why'),   what:_t('inv.s.lmf.what'),   scenarios:_t('inv.s.lmf.scenarios'),   example:_t('inv.s.lmf.example') },
                    { name:_t('inv.s.dmf.name'),   icon:'📊', returns:'6 – 8.5%',        risk:2, riskLabel:_t('inv.risk.l'),  why:_t('inv.s.dmf.why'),   what:_t('inv.s.dmf.what'),   scenarios:_t('inv.s.dmf.scenarios'),   example:_t('inv.s.dmf.example') },
                    { name:_t('inv.s.nsc.name'),   icon:'📜', returns:'7.7%',            risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.nsc.why'),   what:_t('inv.s.nsc.what'),   scenarios:_t('inv.s.nsc.scenarios'),   example:_t('inv.s.nsc.example') },
                    { name:_t('inv.s.potd.name'),  icon:'📮', returns:'6.9 – 7.5%',      risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.potd.why'),  what:_t('inv.s.potd.what'),  scenarios:_t('inv.s.potd.scenarios'),  example:_t('inv.s.potd.example') },
                    { name:_t('inv.s.pomis.name'), icon:'📬', returns:'7.4%',            risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.pomis.why'), what:_t('inv.s.pomis.what'), scenarios:_t('inv.s.pomis.scenarios'), example:_t('inv.s.pomis.example') },
                    { name:_t('inv.s.scss.name'),  icon:'👴', returns:'8.2%',            risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.scss.why'),  what:_t('inv.s.scss.what'),  scenarios:_t('inv.s.scss.scenarios'),  example:_t('inv.s.scss.example') },
                    { name:_t('inv.s.kvp.name'),   icon:'🌾', returns:'7.5%',            risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.kvp.why'),   what:_t('inv.s.kvp.what'),   scenarios:_t('inv.s.kvp.scenarios'),   example:_t('inv.s.kvp.example') },
                    { name:_t('inv.s.rbifr.name'), icon:'🏛️', returns:'7.35% (floating)', risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.s.rbifr.why'), what:_t('inv.s.rbifr.what'), scenarios:_t('inv.s.rbifr.scenarios'), example:_t('inv.s.rbifr.example') },
                ];
            } else {
                return [
                    { name:_t('inv.l.emf.name'),   icon:'📈', returns:'12 – 15%', risk:4, riskLabel:_t('inv.risk.h'),  why:_t('inv.l.emf.why'),   what:_t('inv.l.emf.what'),   scenarios:_t('inv.l.emf.scenarios'),   example:_t('inv.l.emf.example') },
                    { name:_t('inv.l.elss.name'),  icon:'🧾', returns:'12 – 15%', risk:4, riskLabel:_t('inv.risk.h'),  why:_t('inv.l.elss.why'),  what:_t('inv.l.elss.what'),  scenarios:_t('inv.l.elss.scenarios'),  example:_t('inv.l.elss.example') },
                    { name:_t('inv.l.ppf.name'),   icon:'🏛️', returns:'7.1%',     risk:1, riskLabel:_t('inv.risk.vl'), why:_t('inv.l.ppf.why'),   what:_t('inv.l.ppf.what'),   scenarios:_t('inv.l.ppf.scenarios'),   example:_t('inv.l.ppf.example') },
                    { name:_t('inv.l.nps.name'),   icon:'🎯', returns:'8 – 12%',  risk:3, riskLabel:_t('inv.risk.m'),  why:_t('inv.l.nps.why'),   what:_t('inv.l.nps.what'),   scenarios:_t('inv.l.nps.scenarios'),   example:_t('inv.l.nps.example') },
                    { name:_t('inv.l.gold.name'),  icon:'🥇', returns:'9 – 11%',  risk:2, riskLabel:_t('inv.risk.lm'), why:_t('inv.l.gold.why'),  what:_t('inv.l.gold.what'),  scenarios:_t('inv.l.gold.scenarios'),  example:_t('inv.l.gold.example') },
                    { name:_t('inv.l.nifty.name'), icon:'🔷', returns:'11 – 13%', risk:3, riskLabel:_t('inv.risk.m'),  why:_t('inv.l.nifty.why'), what:_t('inv.l.nifty.what'), scenarios:_t('inv.l.nifty.scenarios'), example:_t('inv.l.nifty.example') },
                    { name:_t('inv.l.hybrid.name'),icon:'⚖️', returns:'10 – 12%', risk:3, riskLabel:_t('inv.risk.m'),  why:_t('inv.l.hybrid.why'),what:_t('inv.l.hybrid.what'),scenarios:_t('inv.l.hybrid.scenarios'),example:_t('inv.l.hybrid.example') },
                    { name:_t('inv.l.reit.name'),  icon:'🏢', returns:'8 – 12%',  risk:3, riskLabel:_t('inv.risk.m'),  why:_t('inv.l.reit.why'),  what:_t('inv.l.reit.what'),  scenarios:_t('inv.l.reit.scenarios'),  example:_t('inv.l.reit.example') },
                ];
            }
        }
        ...
        function saveGoalToFP() {
            ...
            var existing = idx >= 0 ? window._savedGoals[idx] : null;
            var entry = { gpType:gpType, fpType:fpType, label:label, emoji:emoji,
                          ...
                          savedAmt:  (existing && existing.savedAmt)  || 0,
                          checkIns:  (existing && existing.checkIns)  || [] };
        ...
        function fpInitQuestions() {
            ...
            fpQuestions.forEach(function(q, qi) {
                ...
                div.innerHTML = '<p class="text-sm font-bold text-slate-700 flex items-start gap-2"><span>' + _fpIco(q.icon) + '</span><span>Q' + (qi+1) + '. ' + qText + '</span></p>' +
            ...
            if (listEl) {
                if (fpState.goals.length === 0) {
                    ...
                    listEl.innerHTML = fpState.goals.map(function(g) {
                        return '<div class="text-xs font-bold text-violet-600">' + _fpIco(g.emoji) + ' ' + g.label + (g.years ? ' · ' + g.years + 'yr' : '') + '</div>';
                    }).join('');
                }
            }
        ...
        function fpCalculatePlan() {
            ...
            (function(){
                ...
                var _pc=_sipFV+existingFutureVal;
                var _s=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};
                _s('fp-kpi-sip',monthlyInvest>0?'₹'+fmt(monthlyInvest):'—');
                _s('fp-kpi-return',(profile.blendedReturn||0).toFixed(1).replace(/\.0$/,'')+'%');
                _s('fp-kpi-horizon',yearsToRetire+' yrs');
                _s('fp-kpi-corpus',_pc>0?_cr(_pc):'—');
            })();
                ...
                if (hasExisting) {
                    var existingRows = fpState.existing.map(function(key) {
                        ...
                        if (key === 'epf') {
                            ...
                            // EPF contributions stop at retirement: monthly additions
                            // accrue only until then, after which the accumulated corpus
                            // earns interest only (no new principal).
                            var accYrs  = Math.min(yearsToRetire, longestYrs);
                            var postYrs = Math.max(0, longestYrs - accYrs);
                            var balFV = val * Math.pow(1 + r, longestYrs);
                            var contribAtRetire = monthly > 0 ? monthly * 12 * ((Math.pow(1 + r, accYrs) - 1) / r) * (1 + r/12) : 0;
                            var sipFV = contribAtRetire * Math.pow(1 + r, postYrs);
                            fv = balFV + sipFV;
                            if (monthly > 0) {
                            ...
                            return '<div class="rounded-xl border border-blue-100 p-2.5 mb-1" style="background:rgba(96,165,250,0.10);">' +
                                ...
                                '<div class="mt-1.5 text-[10px] text-blue-600 leading-relaxed">' +
                                    (monthly > 0 ? '📥 Ongoing: ₹' + fmt(monthly) + '/mo into EPF (you + employer, excl. EPS) until retirement (age ' + retireAge + ')<br>' : '') +
                                    (eps > 0     ? '🧓 EPS pension component: ₹' + fmt(eps) + '/mo (builds pension, not EPF balance)<br>' : '') +
                                    (postYrs > 0 ? '⏸️ After retirement: contributions stop — corpus earns interest only for ' + postYrs + ' more yr' + (postYrs !== 1 ? 's' : '') + '<br>' : '') +
            ...
            if (goalSIPs && monthlyInvest > 0) {
                var reasonLines = goalSIPs.map(function(g) {
                    ...
                    return '<div class="flex items-start gap-2 text-[11px] text-slate-500 leading-relaxed">' +
                        ...
                        '<span><strong style="color:rgba(242,245,240,.9);">' + g.weightPct + '%</strong> → ' + g.goalLabel + ': ' + weightNote + '. Projected at ' + rateNote + '.</span></div>';
                }).join('');
                sipDiv.innerHTML =
                    ...
                    goalSIPs.map(function(g) {
                        return '<div class="fp-sip-row" style="border-left:3px solid ' + g.color + ';padding-left:10px;">' +
                            '<div class="flex items-center gap-2"><span class="text-sm" style="color:rgba(242,245,240,.85);display:inline-flex;align-items:center;gap:6px;">' + _fpIco(g.emoji) + window.esc(g.goalLabel || g.label) + '</span><span class="text-[10px] text-slate-400">' + g.years + 'yr horizon</span></div>' +
            ...
            if (redeemPlan.length > 0) {
                ...
                var liqLabel = { instant:'<span class="fp-dot" style="background:#34d399"></span>'+_t('fp.redeem.liq.instant'), high:'<span class="fp-dot" style="background:#fbbf24"></span>'+_t('fp.redeem.liq.high'), medium:'<span class="fp-dot" style="background:#fb923c"></span>'+_t('fp.redeem.liq.medium'), low:'<span class="fp-dot" style="background:#f87171"></span>'+_t('fp.redeem.liq.low') };
            ...
            document.getElementById('fp-roadmap').innerHTML = roadmap.map(function(r) {
                ...
                return '<div class="fp-road-item" style="background:' + r.color + '0f;border-color:' + r.color + '30;"><span class="text-lg flex-shrink-0">' + _fpIco(r.icon) + '</span><div><div class="text-xs font-black uppercase tracking-wide mb-0.5" style="color:' + r.color + '">' + rTitle + '</div><div class="text-xs text-slate-600 leading-relaxed">' + rDesc + '</div></div></div>';
            }).join('');
