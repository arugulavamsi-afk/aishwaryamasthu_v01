...
function cgCalc() {
    ...
    if (!buyStr || !sellStr || !cost || !salePrice) {
        resEl.innerHTML = '<p class="text-[11px] text-slate-400 text-center py-6 font-semibold">' + T('cg.placeholder', 'Fill in all fields to calculate your capital gains tax') + '</p>';
        return;
    }
    ...
    if (isNaN(buyDate) || isNaN(sellDate) || sellDate <= buyDate) {
        resEl.innerHTML = '<p class="text-[11px] text-red-500 text-center py-4 font-semibold">' + T('cg.err.date', 'Sale date must be after purchase date') + '</p>';
        return;
    }
    ...
    var holdLabel = holdYr > 0
        ? holdYr + ' ' + (holdYr > 1 ? T('cg.res.yrs', 'yrs') : T('cg.res.yr', 'yr')) + (holdMo > 0 ? ' ' + holdMo + ' ' + T('cg.res.mo', 'mo') : '')
        : holdMonths + ' ' + (holdMonths !== 1 ? T('cg.res.months', 'months') : T('cg.res.month', 'month'));
    ...
    switch (asset) {
        ...
        case 'equity':
            ...
            if (isLTCG) {
                ...
                note = sellDate >= JUL23_2024
                    ? T('cg.note.eq.ltcg.post', 'LTCG 12.5% (Budget 2024). ₹1.25L/yr exemption, ₹{remaining} remaining.').replace('{remaining}', remaining.toLocaleString('en-IN'))
                    : T('cg.note.eq.ltcg.pre',  'LTCG 10% (pre-Jul 23, 2024). ₹1L/yr exemption, ₹{remaining} remaining.').replace('{remaining}', remaining.toLocaleString('en-IN'));
            } else {
                taxType = 'STCG'; section = 'Sec 111A';
                taxRate = sellDate >= JUL23_2024 ? 0.20 : 0.15;
                note = sellDate >= JUL23_2024
                    ? T('cg.note.eq.stcg.post', 'STCG 20% (raised from 15% on Jul 23, 2024).')
                    : T('cg.note.eq.stcg.pre',  'STCG 15% (pre-Jul 23, 2024 rate).');
            }
            break;
        ...
        case 'debt-new':
            ...
            note    = T('cg.note.debt.new', 'Debt / Intl MF purchased after Apr 1, 2023: all gains taxed at slab rate, no indexation, no LTCG benefit (Finance Act 2023).');
            break;
        ...
        case 'debt-old':
            ...
            if (isLTCG) {
                ...
                taxRate     = 0.20;
                showIdx     = true;
                note = T('cg.note.debt.old.ltcg', 'Grandfathered debt MF (pre Apr 1, 2023): LTCG 20% with indexation. CII Buy {bCII} → Sell {sCII}.')
                    .replace('{bCII}', bCII).replace('{sCII}', sCII);
            } else {
                taxType = 'STCG'; section = 'Sec 112';
                taxRate = slab / 100;
                note    = T('cg.note.debt.old.stcg', 'Debt MF (pre Apr 1, 2023): STCG < 36 months taxed at slab rate.');
            }
            break;
            ...
            if (isLTCG) {
                ...
                if (isPreJul23) {
                    ...
                    if (t12 <= t20) {
                        ...
                        altTax  = { label: T('cg.alt.20idx', '20% + indexation'), tax: t20, idxCost: idxCost };
                        note    = T('cg.note.gold.125', '12.5% (no indexation) is lower — selected. CII {bCII}→{sCII}. You may also opt for 20% with indexation if preferred.')
                            .replace('{bCII}', bCII).replace('{sCII}', sCII);
                    } else {
                        taxRate = 0.20; taxable = Math.max(0, salePrice - idxCost); indexedCost = idxCost;
                        altTax  = { label: T('cg.alt.125noidx', '12.5% (no indexation)'), tax: t12, idxCost: null };
                        note    = T('cg.note.gold.20', '20% with indexation is lower — selected. CII {bCII}→{sCII}.')
                            .replace('{bCII}', bCII).replace('{sCII}', sCII);
                    }
                } else {
                    taxRate = 0.125; taxable = Math.max(0, gain);
                    note    = T('cg.note.gold.post', 'Gold purchased after Jul 23, 2024: LTCG 12.5%, no indexation, 24-month threshold (Finance Act 2024).');
                }
            } else {
                taxType = 'STCG'; section = 'Sec 112'; taxRate = slab / 100;
                note    = T('cg.note.gold.stcg', 'Gold STCG taxed at income slab rate.');
            }
            break;
            ...
            if (isLTCG) {
                ...
                if (isPreJul23) {
                    ...
                    if (t12 <= t20) {
                        taxRate = 0.125; taxable = Math.max(0, gain);
                        altTax  = { label: T('cg.alt.20idx', '20% + indexation'), tax: t20, idxCost: idxCost };
                        note    = T('cg.note.re.125', '12.5% (no indexation) is lower — selected. CII {bCII}→{sCII}. Also check Sec 54 / 54EC exemptions for reinvestment.')
                            .replace('{bCII}', bCII).replace('{sCII}', sCII);
                    } else {
                        taxRate = 0.20; taxable = Math.max(0, salePrice - idxCost); indexedCost = idxCost;
                        altTax  = { label: T('cg.alt.125noidx', '12.5% (no indexation)'), tax: t12, idxCost: null };
                        note    = T('cg.note.re.20', '20% with indexation is lower — selected. CII {bCII}→{sCII}. Also check Sec 54 / 54EC exemptions.')
                            .replace('{bCII}', bCII).replace('{sCII}', sCII);
                    }
                } else {
                    taxRate = 0.125; taxable = Math.max(0, gain);
                    note    = T('cg.note.re.post', 'Property purchased after Jul 23, 2024: LTCG 12.5%, no indexation. Check Sec 54 / 54EC reinvestment exemptions.');
                }
            } else {
                taxType = 'STCG'; section = 'Sec 112'; taxRate = slab / 100;
                note    = T('cg.note.re.stcg', 'Property STCG taxed at slab rate. Sec 54 exemption applies only to LTCG.');
            }
            break;
            ...
            if (isLTCG) {
                ...
                if (isPreJul23) {
                    ...
                    if (t12 <= t20) {
                        taxRate = 0.125; taxable = Math.max(0, gain);
                        altTax  = { label: T('cg.alt.20idx', '20% + indexation'), tax: t20, idxCost: idxCost };
                        note    = T('cg.note.unl.125', '12.5% (no indexation) is lower — selected. Finance Act 2024.');
                    } else {
                        taxRate = 0.20; taxable = Math.max(0, salePrice - idxCost); indexedCost = idxCost; showIdx = true;
                        altTax  = { label: T('cg.alt.125noidx', '12.5% (no indexation)'), tax: t12, idxCost: null };
                        note    = T('cg.note.unl.20', '20% with indexation is lower — selected.');
                    }
                } else {
                    taxRate = 0.125; taxable = Math.max(0, gain);
                    note    = T('cg.note.unl.post', 'Unlisted shares (post Jul 23, 2024): LTCG 12.5%, no indexation.');
                }
            } else {
                taxType = 'STCG'; section = 'Sec 112'; taxRate = slab / 100;
                note    = T('cg.note.unl.stcg', 'Unlisted shares STCG taxed at slab rate.');
            }
            break;
        ...
        case 'sgb':
            ...
            if (holdYearsRaw >= 8) {
                ...
                taxRate = 0; taxable = 0;
                note = T('cg.note.sgb.exempt', 'SGB held to maturity (8 years): capital gains fully exempt for original allottees. Secondary market buyers: equity capital gains treatment applies.');
            } else {
                ltcgMonths = 12;
                isLTCG = holdMonths >= ltcgMonths;
                if (isLTCG) {
                    ...
                    taxable = Math.max(0, gain - exempt);
                    note    = T('cg.note.sgb.ltcg', 'SGB transferred before maturity: equity LTCG treatment. {rate}% above ₹{limit} exemption.')
                        .replace('{rate}', taxRate * 100)
                        .replace('{limit}', (sgbLimit / 100000).toFixed(2) + 'L');
                } else {
                    taxType = 'STCG'; section = 'Sec 111A';
                    taxRate = sellDate >= JUL23_2024 ? 0.20 : 0.15;
                    note    = T('cg.note.sgb.stcg', 'SGB < 12 months: equity STCG at {rate}%.')
                        .replace('{rate}', taxRate * 100);
                }
            }
            break;
    ...
    var holdTypeLabel = isExempt
        ? T('cg.res.exempt.mat', 'Exempt at Maturity')
        : (isLTCG ? T('cg.res.longterm', 'Long-Term') : (asset === 'debt-new' ? T('cg.res.slabrate', 'Slab Rate') : T('cg.res.shortterm', 'Short-Term')));
    ...
    h +=     '<div class="text-[9px] font-black uppercase tracking-wider mb-0.5" style="color:' + ltcgFg + ';">' + T('cg.res.holdperiod', 'Holding Period') + '</div>';
    h +=     '<div class="text-base font-black" style="color:' + ltcgFg + ';">' + holdLabel + '</div>';
    h +=     '<div class="text-[9px] font-semibold" style="color:' + ltcgFg + ';">' + holdTypeLabel + ' · ' + section + '</div>';
    h +=   '</div>';
    h +=   '<div class="rounded-xl px-2.5 py-2 text-center" style="background:' + gainBg + ';border:1px solid ' + gainBdr + ';">';
    h +=     '<div class="text-[9px] font-black uppercase tracking-wider mb-0.5" style="color:' + gainColor + ';">' + (isLoss ? T('cg.res.capitalloss', 'Capital Loss') : T('cg.res.capitalgain', 'Capital Gain')) + '</div>';
    h +=     '<div class="text-base font-black" style="color:' + gainColor + ';">' + _cgInr(gain) + '</div>';
    h +=     '<div class="text-[9px] font-semibold" style="color:' + gainColor + ';">' + T('cg.res.sale', 'Sale') + ' ' + _cgInr(salePrice) + ' − ' + T('cg.res.cost', 'Cost') + ' ' + _cgInr(cost) + '</div>';
    h +=   '</div>';
    h += '</div>';
    ...
    if (isLoss) {
        ...
        h += '<strong>' + T('cg.res.capitalloss', 'Capital Loss') + ': ' + _cgInr(Math.abs(gain)) + '</strong> — ' + T('cg.res.loss.notax', 'No tax payable.') + ' ';
        h += asset === 'equity'
            ? T('cg.res.equity.loss', 'STCG losses offset any CG; LTCG losses only offset LTCG. Both carry forward 8 years.')
            : T('cg.res.other.loss', 'Can be set off against gains of same type. Carry forward up to 8 years (file ITR before due date).');
        h += '</div>';
    ...
    if (isExempt) {
        ...
        h += '✅ <strong>' + T('cg.res.capitalgain', 'Capital Gain') + ' ' + _cgInr(gain) + ' ' + T('cg.res.exempt.text', 'is fully exempt from tax.') + '</strong> ' + note;
        h += '</div>';
        resEl.innerHTML = h; return;
    }
    ...
    h += row(T('cg.res.purchase.cost', 'Purchase Cost'), _cgInr(cost));
    if (showIdx && indexedCost > cost) {
        h += row(T('cg.res.indexed.cost', 'Indexed Cost (CII adj.)'), _cgInr(indexedCost), '#7c3aed');
    }
    if (exempt > 0) {
        var exemptLabel = T('cg.res.exemption', 'Exemption') + (asset === 'equity' || asset === 'sgb' ? ' (' + T('cg.res.limit.125', '₹1.25L limit') + ')' : '');
        h += row(exemptLabel, '− ' + _cgInr(exempt), '#059669');
    }
    h += row(T('cg.res.taxable.gain', 'Taxable Gain'), _cgInr(taxable));
    h += row(T('cg.res.tax.rate', 'Tax Rate') + ' (' + taxType + ')', (taxRate * 100).toFixed(1) + '%', '#dc2626');
    h += row(T('cg.res.cg.tax', 'Capital Gains Tax'), _cgInr(tax), '#dc2626');
    h += row(T('cg.res.cess', 'Health & Edu. Cess (4%)'), _cgInr(cess), '#b45309');
    h += '</div>';
    ...
    h +=     '<div class="text-[9px] font-black text-red-600">' + T('cg.res.total.tax', 'Total Tax + Cess') + '</div>';
    h +=     '<div class="text-sm font-black text-red-700">' + _cgInr(totalTax) + '</div>';
    h +=     '<div class="text-[9px] text-red-400">' + effRate.toFixed(1) + T('cg.res.pct.gain', '% of gain') + '</div>';
    h +=   '</div>';
    h +=   '<div class="rounded-xl p-2 text-center" style="background:#f0fdf4;border:1px solid #86efac;">';
    h +=     '<div class="text-[9px] font-black text-emerald-600">' + T('cg.res.net.proc', 'Net Proceeds') + '</div>';
    h +=     '<div class="text-sm font-black text-emerald-700">' + _cgInr(netProc) + '</div>';
    h +=     '<div class="text-[9px] text-emerald-400">' + T('cg.res.after.tax', 'after all tax') + '</div>';
    h +=   '</div>';
    h +=   '<div class="rounded-xl p-2 text-center" style="background:#eff6ff;border:1px solid #bfdbfe;">';
    h +=     '<div class="text-[9px] font-black text-blue-600">' + T('cg.res.posttax', 'Post-Tax Profit') + '</div>';
    h +=     '<div class="text-sm font-black text-blue-700">' + _cgInr(gain - totalTax) + '</div>';
    h +=     '<div class="text-[9px] text-blue-400">' + T('cg.res.gain.tax', 'gain − tax') + '</div>';
    h +=   '</div>';
    h += '</div>';
    ...
    if (altTax) {
        ...
        h += '<strong>' + T('cg.res.alt.opt', 'Alternative option:') + '</strong> ' + altTax.label + ' = ' + _cgInr(altTax.tax);
        if (altTax.idxCost) h += ' (' + T('cg.res.idx.cost.label', 'indexed cost') + ' ' + _cgInr(altTax.idxCost) + ')';
        h += '. ' + T('cg.res.alt.note', 'The calculator auto-selects the lower option. Confirm with your CA before filing.');
        h += '</div>';
    }
