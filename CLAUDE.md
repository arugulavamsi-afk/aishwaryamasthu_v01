...
- **Routing:** new tools must be registered in `app.js` (`switchMode()`) before anything else works. Never assign `currentMode` directly — always use `_setMode()`, which mirrors it to `window._currentMode` (other modules gate their post-Firestore re-renders on that global)
...
- **Firestore saves:** always go through `saveUserData()` in `auth.js` (debounced, merged writes). Writes are gated on `_loadComplete` — never write before the initial read resolves, or empty in-memory state overwrites saved data. When assembling the save payload, unconditional keys (`userProfile`, `growth`, `goal`, `emergency`, `savedGoals`, `roadmap`) must use `_orBase(current, base, fallback)`, never `||` — an empty `{}`/`[]` is truthy and would clobber the user's data. Tool data lives in per-tool docs `users/{uid}/tools/{toolId}` (payload under field `v`), never on the user doc — the user doc holds only small account-level fields (`fname`, `riskProfile`, `toolSummaries`). Legacy `appData` is auto-migrated on login
